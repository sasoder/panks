...
class Game {
  constructor(roomID, players, width, height, amp) {
    ...
    this.width = width;
    this.height = height;
    this.skyColour = this.skyColours[
      Math.floor(Math.random() * this.skyColours.length)
    ];
    this.groundColor = this.groundColours[
      Math.floor(Math.random() * this.groundColours.length)
    ];
    this.over = false;
    ...
    switch (this.players.length) {
      ...
      default:
        if (playerIndex > this.currentPlayerIndex) {
          // if the player who left is AFTER the current player
  ...
  update() {
    this.players.forEach((player) =>
      player.update(this.gravity, this.gameScreen, this.width, this.height)
    );
    ...
    if (this.currentPlayer != undefined) {
      if (
        (this.bullets.length === 0 && this.currentPlayer.shots === 0) ||
  ...
  checkWin() {
    ...
    if (alivePlayers.length == 1) {
      if (!this.over) {
        ...
        if (this.currentPlayer != undefined)
          model.updatePlayerStats(this.currentPlayer, true);
      }
      clearInterval(this.decInt);
      return true;
    }
  ...
  checkCollisions() {
    this.players.forEach((p) => {
      ...
      this.bullets.forEach((bullet, i) => {
        if (
          this.terrainCollision(bullet) ||
          this.checkEntityCollision(p, bullet)
        ) {
          bullet.explode(
            this.players,
            this.gameScreen,
  ...
  checkEntityCollision(entity1, entity2) {
    ...
    return (
      cent[0] >= entity1.x &&
      cent[0] <= entity1.x + entity1.width &&
      cent[1] >= entity1.y &&
      cent[1] <= entity1.y + entity1.height
    );
  }

  terrainCollision(entity) {
    if (
      Math.round(entity.x) <= 0 ||
      Math.round(entity.x + entity.width) >= this.width
    )
      return true;

    if (Math.round(entity.y) > this.height) return true;

    // if bullet is above game screen it can still move
    if (Math.round(entity.y) < 0) return false;

    return this.gameScreen[Math.round(centerOfObject(entity)[0])][
      Math.round(entity.y + entity.height)
    ];
  }
  ...
  countDownCurrentPlayerTurn() {
    this.decInt = setInterval(() => {
      ...
      if (--this.currentPlayer.timeLeft <= 0) {
        ...
        if (this.bullets.length === 0) this.changeTurn();
      }
    }, 1000);
  }
  ...
  cleanTerrain() {
    for (let col = 0; col < this.width; col++) {
      ...
      for (let row = this.height - 1; row >= 0; row--) {
        if (!this.gameScreen[col][row]) {
          // first occurence of air
          countDrops = true;
          if (firstAir == this.height) firstAir = row;
        }
